...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        byte[] rateGrnL = new byte[1];
        byte[] rateGrnR = new byte[1];

        byte[] rateBlu = new byte[1];
        byte[] rateBluL = new byte[1];
        byte[] rateBluR = new byte[1];
            ...
            */
            ...
            if (worldGrid.isRateTrigger && worldGrid.isRateMap)
            {
                ...
                if (worldGrid.numRateChannels > 0)
                {
                    ...
                    GL.ReadPixels(ratePixelsCenter, rateLookaheadCenter, 1, 1, OpenTK.Graphics.OpenGL.PixelFormat.Green, PixelType.UnsignedByte, rateGrn);
                    GL.ReadPixels(ratePixelsCenterL, rateLookaheadCenterL, 1, 1, OpenTK.Graphics.OpenGL.PixelFormat.Green, PixelType.UnsignedByte, rateGrnL);
                    GL.ReadPixels(ratePixelsCenterR, rateLookaheadCenterR, 1, 1, OpenTK.Graphics.OpenGL.PixelFormat.Green, PixelType.UnsignedByte, rateGrnR);

                    GL.ReadPixels(ratePixelsCenter, rateLookaheadCenter, 1, 1, OpenTK.Graphics.OpenGL.PixelFormat.Blue, PixelType.UnsignedByte, rateBlu);
                    GL.ReadPixels(ratePixelsCenterL, rateLookaheadCenterL, 1, 1, OpenTK.Graphics.OpenGL.PixelFormat.Blue, PixelType.UnsignedByte, rateBluL);
                    GL.ReadPixels(ratePixelsCenterR, rateLookaheadCenterR, 1, 1, OpenTK.Graphics.OpenGL.PixelFormat.Blue, PixelType.UnsignedByte, rateBluR);
                }
                ...
                byte rate = (byte)(Math.Round((double)(rateRed[0]), MidpointRounding.AwayFromZero));
                byte rateL = (byte)(Math.Round((double)(rateRedL[0]), MidpointRounding.AwayFromZero));
                byte rateR = (byte)(Math.Round((double)(rateRedR[0]), MidpointRounding.AwayFromZero));
                byte rateAvg = (byte)((rateL * 0.3) + (rate * 0.4) + (rateR * 0.3));
                btnSection1Man.Text = rateAvg.ToString();

                p_228.pgn[p_228.rate0] = rateAvg;

                rate = (byte)(Math.Round((double)(rateGrn[0]), MidpointRounding.AwayFromZero));
                rateL = (byte)(Math.Round((double)(rateGrnL[0]), MidpointRounding.AwayFromZero));
                rateR = (byte)(Math.Round((double)(rateGrnR[0]), MidpointRounding.AwayFromZero));
                rateAvg = (byte)((rateL * 0.3) + (rate * 0.4) + (rateR * 0.3));
                btnSection2Man.Text = rateAvg.ToString();

                p_228.pgn[p_228.rate1] = rateAvg;

                rate = (byte)(Math.Round((double)(rateBlu[0]), MidpointRounding.AwayFromZero));
                rateL = (byte)(Math.Round((double)(rateBluL[0]), MidpointRounding.AwayFromZero));
                rateR = (byte)(Math.Round((double)(rateBluR[0]), MidpointRounding.AwayFromZero));
                rateAvg = (byte)((rateL * 0.3) + (rate * 0.4) + (rateR * 0.3));
                btnSection3Man.Text = rateAvg.ToString();

                p_228.pgn[p_228.rate2] = rateAvg;
