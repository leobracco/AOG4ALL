...
namespace AgOpenGPS
{
    ...
    public partial class FormGPS : Form
    {
        ...
        private TableLayoutPanel panelSensors;
        private int numberOfSensors = 32;
        ...
        public FormGPS()
        {
            ...
            _sensorConfig = LoadSensorConfig();
            InitializeComponent();
            ...
            {
                // Usar la configuración cargada, por ejemplo:
                numberOfSensors = _sensorConfig.NumberOfSensors;
                // y actualizar la interfaz de usuario según sea necesario
                
            }
            panelSensors = new TableLayoutPanel();
            panelSensors.Name = "panelSensors";
            //panelSensors.Size = new Size(100, 200); // Ajusta esto según tus necesidades
            panelSensors.Location = new Point(100, 800); // Ajusta esto según tus necesidades
            panelSensors.BackColor = Color.Transparent; // O el color de fondo deseado

            // Configurar las columnas y filas del TableLayoutPanel
            panelSensors.ColumnCount = numberOfSensors; // Número de columnas
            panelSensors.RowCount = 1; // Número de filas
            

            // Añadir panelSensors al formulario
            Controls.Add(panelSensors);
            panelSensors.BringToFront(); // Esto asegura que el panel esté
            //CreateSensorPanel(numberOfSensors);
            //time keeper
            secondsSinceStart = (DateTime.Now - Process.GetCurrentProcess().StartTime).TotalSeconds;
            AddControlsToPanelSensors(numberOfSensors);
        ...
        private SensorConfig LoadSensorConfig()
        {
            try
            {
                string filePath = "config.json";
                string json = File.ReadAllText(filePath);
                SensorConfig config = JsonConvert.DeserializeObject<SensorConfig>(json);
                return config;
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error loading sensor configuration: " + ex.Message);
                return null;
            }
        }
        private void btnAGPConfig_Click(object sender, EventArgs e)
        {
            Form_AGPConfig configForm = new Form_AGPConfig(this);
            configForm.ShowDialog(); // Abre el Form_AGPConfig como un diálogo modal
        }

        private void SeedSensorButton_Click(object sender, EventArgs e)
        {
            Button btn = sender as Button;
            MessageBox.Show("Sensor " + btn.Name + " clicked!");
            TestButtonFind();
        }
        private void InitializeWebSocket()
        {
            _webSocketClient = new WebSocketClient(this.panelSensors);

             
            Task.Run(() => _webSocketClient.ConnectAsync("ws://127.0.0.1:8080"));  // Reemplaza la URL por tu URI de WebSocket
        }
        public void LoadAndApplySensorConfig()
        {
            _sensorConfig = LoadSensorConfig();
            if (_sensorConfig != null)
            {
                ApplySensorConfig(_sensorConfig);
            }
            else
            {
                MessageBox.Show("Failed to load sensor configuration.");
            }
        }

        private void ApplySensorConfig(SensorConfig config)
        {
            // Actualizar el número de sensores
            AddControlsToPanelSensors(config.NumberOfSensors);
        }
        public void AddControlsToPanelSensors(int numberOfSensors)
        {
            // Asegúrate de limpiar el panel antes de añadir nuevos controles
            panelSensors.Controls.Clear();
            panelSensors.ColumnStyles.Clear(); // Limpiar estilos de columnas existentes
            panelSensors.Size = new System.Drawing.Size(49* numberOfSensors, 49);  // Ajusta el tamaño según tus necesidades
            //panelSensors.Size = AutoSize;
            panelSensors.TabIndex = 326;  // Asegúrate de que 
            // Definir el número de columnas
            panelSensors.ColumnCount = numberOfSensors;
            panelSensors.RowCount = 1;
            // Ajustar los estilos de columnas
            for (int i = 0; i < numberOfSensors; i++)
            {
                // Añadir un nuevo estilo para cada columna
                panelSensors.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,  49));
               
                //Console.WriteLine($"{DateTime.Now}:Style - {SizeType.Percent}");
            }
            ...
            for (int i = 0; i < numberOfSensors; i++)
            {
                ...
                btn.Text = $"{i + 1}";
                btn.Dock = DockStyle.Fill; // Esto hará que el botón llene la celda
                btn.Name = $"AGPSeed{i + 1}";
                btn.Enabled = true;
                ...
                btn.Font = new System.Drawing.Font("Tahoma", 8F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
                btn.ForeColor = System.Drawing.Color.White;
                btn.ImeMode = System.Windows.Forms.ImeMode.NoControl;
                btn.Location = new System.Drawing.Point(4, 4);
                
                btn.Size = new System.Drawing.Size(6, 41);
                btn.TabIndex = 543;
                btn.UseVisualStyleBackColor = false;
                btn.Click += new EventHandler(SeedSensorButton_Click);
                panelSensors.Controls.Add(btn, i, 0); // Añadir botón en la columna 'i', fila 0
            }

            // Reorganizar el layout para reflejar los cambios
            panelSensors.PerformLayout();
            
        }
        private void TestButtonFind()
        {
            Button testBtn = panelSensors.Controls.Find("AGPSeed1", true).FirstOrDefault() as Button;
            if (testBtn != null)
            {
                MessageBox.Show("Button found!");
            }
            else
            {
        ...
        {
            // Centrar panelSensors horizontalmente
            panelSensors.Left = (this.ClientSize.Width - panelSensors.Width) / 2;
        ...
        private void FormGPS_FormClosingWS(object sender, FormClosingEventArgs e)
        {
